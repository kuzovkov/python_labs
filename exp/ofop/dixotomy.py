...
root.title('Метод дихотомии')

#globals variables
d = 0.000001 #отступ точек от центра интервала
...
def nextIt():
    global eps,a,b,colors,count,lines,textbox,d,epsCurrent
    x1 = a + ( b - a )/2 - d
    x2 = a + ( b - a )/2 + d
    #print count, 'a= ', a,' b= ', b,' x1 = ', x1, ' x2= ', x2, ' F((a+b)/2)= ', F( (a+b) / 2 )
...
def writeEps():
    ...
    if epsCurrent <= eps:
        ...
        fg = 'red'
    labelEps.configure( text='интервал неопред.: ' + fpformat.fix(float(epsCurrent),6), fg = fg)
