...
def show_start(): #показ стартовой формы
    ...
    pole=Entry(frm,bd=5,width=30, font='Arial 14', textvariable=user_name).pack()
    next_btn=Button(frm,bd=5,text='Начать тестирование', font='Arial 20',bg=bg4,fg=fg6, command=start).pack()
    end_btn=Button(frm,bd=5,text='Выход', font='Arial 20', bg=bg5,fg=fg6,command=end).pack()
...
def show_form_str(): #показ формы строкового ответа
    ...
    next_btn=Button(frm,bd=5,text='Отправить ответ', font='Arial 20', command=send_str,bg=bg4,fg=fg6).pack()
    end_btn=Button(frm,bd=5,text='Выход', font='Arial 20', command=end,bg=bg5,fg=fg6).pack()
    rsp_text.set('')
...
def show_form_num(): #показ формы числового ответа
    ...
    next_btn=Button(frm,bd=5,text='Отправить ответ', bg=bg4,fg=fg6,font='Arial 20', command=send_num).pack()
    end_btn=Button(frm,bd=5,text='Выход', bg=bg5,fg=fg6,font='Arial 20', command=end).pack()
    rsp_text.set('')
...
def show_form_and(): #показ формы  ответа типа And
    ...
    next_btn=Button(frm,bd=5,bg=bg4,fg=fg6,text='Отправить ответ', font='Arial 20', command=send_and).pack()
    end_btn=Button(frm,bd=5,bg=bg5,fg=fg6,text='Выход', font='Arial 20', command=end).pack()
...
def show_form_or():  #показ формы  ответа типа Or
    ...
    next_btn=Button(frm,bd=5,bg=bg4,fg=fg6,text='Отправить ответ', font='Arial 20', command=send_or).pack()
    end_btn=Button(frm,bd=5,bg=bg5,fg=fg6,text='Выход', font='Arial 20', command=end).pack()
...
def show_form_end():  #показ формы  результата
    ...
    #кнопки
    start_btn=Button(frm,bd=5,bg=bg4,fg=fg6,text='Повторить тестирование', font='Arial 20', command=start_again).pack()
    reapeat_btn=Button(frm,bd=5,bg=bg6,fg=fg6,text='Новый пользователь', font='Arial 20', command=new_user).pack()
    end_btn=Button(frm,bd=5,bg=bg5,fg=fg6,text='Выход', font='Arial 20', command=end).pack()
