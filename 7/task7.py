...
def send_num(e=0): #обработка числового ответа
    ...
    if not rsp_text.get().isdigit():
        tkMessageBox.showwarning("Тест","Ожидается число!")
        clear_frame()
        show_form_num()
        return
...
def send_and(e=0): #обработка  ответа типа And
    ...
    if rsp==(0,0,0,0):
        if tkMessageBox.askokcancel("Тест","Вы ничего не выбрали, продолжить?"):
            pass
        else:
            clear_frame()
            show_form_and()
...
def show_form_str(): #показ формы строкового ответа
    ...
    pole=Entry(frm,takefocus=TRUE,bd=5,width=30,font='Arial 14',textvariable=rsp_text).pack()
...
def show_form_num(): #показ формы числового ответа
    ...
    number=Label(frm,bg=bg1,fg=fg3,text='Вопрос '+str(i)+' из '+str(len(questions))).pack()
    question=Label(frm,bd=5,text=questions[i],bg=bg2,fg=fg2,width=100,height=5).pack()
    pole_num=Entry(frm,takefocus=TRUE,bd=5,width=30,font='Arial 14',textvariable=rsp_text).pack()
...
def show_form_and(): #показ формы  ответа типа And
    ...
    for item in variants[i]:
        ...
        check.append(cb)
        check[len(check)-1].place(x=30,y=100+j*20)
        j=j+1
...
def show_form_or():  #показ формы  ответа типа Or
    ...
    for item in variants[i]:
        ...
        rbts.append(rb)
        rbts[len(rbts)-1].place(x=30,y=100+j*20)
        j=j+1
