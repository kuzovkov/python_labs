...
reportFile = 'report.count.words2.2.txt' #файл с исключаемыми словами
...
def sortListByCount( ls, sortDirect = True ): #сортировка списка по количеству записей в строке
    indexSorted=[]
    lsSorted=[]
    lsLen=len(ls)
    while len(indexSorted) != lsLen:
...
#сортируем результат
wordTable = sortListByCount( wordTable )
